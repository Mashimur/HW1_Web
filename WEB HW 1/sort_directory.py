...
import os
import shutil
from output_view import SortDirectoryView
from input_validation import NameValidate

class SortDirectory:
    def __init__(self):
        self.result_view = SortDirectoryView()
        self.name = NameValidate()
    ...
    def sort_create_files(self, start_path):
            ...
            for file in files:
                try:
                    ...
                    if list_files[-1].upper() in list_image:
                        new_name = os.path.join(root, self.name.validate(file))
                        ...
                        filename = self.name.validate(file)
        ...
        return print(self.result_view.create_row(result))


def run_sort():
    try:
        first_use = input('Input path: ')
        path = SortDirectory()
        path.sort_create_files(first_use)
        while True:
            b = input("\nDo you want to sort something else? (yes/no)\n>>>> ")
            if b == "yes":
                a = input('Input path: ')
                path = SortDirectory()
                path.sort_create_files(a)
                continue
            if b == "no":
                print("Goodbye!")
                break
