...
from input_validation import BirthdayValidate, PhoneValidate, MailValidate
from output_view import AddressBookView
import pickle
...
class AddressBook(UserDict):
    ...
    def show_rec(self, name):
        ...
        return self.ov.create_row(result)
    ...
    def iterator(self, n=1):
        ...
        for rec in self.data.values():
            if count < n:
                result.append([rec.name, rec.birthday, rec.mail, rec.address, ", ".join([p.value for p in rec.phones])])
                count += 1
        yield self.ov.create_table(result)
...
class Phone(Field):
    ...
    def __init__(self, value):
        ...
        self.phone = PhoneValidate()
        self._value = self.phone.validate(value)
...
class Birthday(datetime):
    ...
    def __init__(self, year, month, day):
        super().__init__()
        self.bday = BirthdayValidate()
        self.__birthday = self.bday.validate(year, month, day)
...
class Mail(Field):
    def __init__(self, value):
        super().__init__(value)
        self.mail = MailValidate()
        self.value = self.mail.validate(value)
    ...
    @value.setter
    def value(self, value):
        self.__value = self.mail.validate(value)
...
class Record:
    ...
    def change(self, old_phone, new_phone):
        ...
        for phone in self.phones:
            if phone.value == old_phone.value:
                ...
                return f"Phone {old_phone} not found in the Record"
    ...
    def add_user_birthday(self, year, month, day):
        self.birthday = self.bday.validate(int(year), int(month), int(day))
